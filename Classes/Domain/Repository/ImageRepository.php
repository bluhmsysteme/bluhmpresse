...
class ImageRepository extends \TYPO3\CMS\Extbase\Persistence\Repository {
    ...
    protected $defaultOrderings = array(
        'image_date' => \TYPO3\CMS\Extbase\Persistence\QueryInterface::ORDER_DESCENDING
     );
    ...
    public function findByFilter($filter) {
        ...
        $constraints = array();
        if($filter->keyword) {
            $constraints[] = $query->like('title', '%'.$filter->keyword.'%');
        }
        if($filter->industry) {
        ...
        if($filter->year) {
            ...
            $constraints[] = $query->greaterThanOrEqual('image_date', $minDate);
            $constraints[] = $query->lessThanOrEqual('image_date', $maxDate);
        }
    ...
    /**
     * Finds years having images mathcing the filter
     *
       @param \Mkuehnel\Bluhmpresse\Domain\Model\SearchFilter $filter
    ...
    public function findYearsByFilter($filter) {
        ...
        $filter2->year = null;
        $images = $this->findByFilter($filter2);
        $yearArray = array();
        foreach($images as $image) {
            $date = $image->getImageDate();
            $year = $date->format('Y');
    ...
    /**
     * Get all articles who have a reference to specific image
     *
     * @param \integer $imageUid image to get refs for
     */
    public function findArticleRefs($imageUid) {
        $tablename = 'tx_bluhmpresse_domain_model_article';
        $ref_table = 'tx_bluhmpresse_domain_model_image';
        $rows = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
            'recuid',
            'sys_refindex',
            'tablename=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($tablename, 'sys_refindex') . ' AND ref_table=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($ref_table, 'sys_refindex') . ' AND ref_uid=' . intval($imageUid) . ' AND deleted=0'
        );
        return $rows;
    }
